sap.ui.define([
	...
	"use strict";
	return e.extend("dksh.connectclient.InventoryAtp.controller.inventory", {
		formatter: o,
		...
		_getUser: function () {
			...
			this._doAjax(e, "GET", "", true).then(e => {
				...
				// [+] START Modification: STRY0014745:MY Enhancements Defaulting mandatory fields -	JAYAMALARJ
				this.clearTabData();
				this._getPersonalizationDetails("keyATPOverviewEnquiry");
				// [+] END Modification: STRY0014745:MY Enhancements Defaulting mandatory fields -	JAYAMALARJ
			}, e => {
				t.close();
		...
		onPressDetailEndStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryAtp.Fragments.EndingStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
		...
		onPressDetailExpStk: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryAtp.Fragments.ExpiryStockPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailMatMov: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryAtp.Fragments.MatMovPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onPressDetailATP: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryAtp.Fragments.ATPPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		onSelectvarian: function (e) {
			...
			this.selectedTab = "keyATPOverviewEnquiry";
			if (e) {
		...
		onPersonlizationClose: function () {
			...
			if (!this.FilterPersonalization) {
				this.FilterPersonalization = sap.ui.xmlfragment("dksh.connectclient.InventoryAtp.Fragments.FilterPersonalization", this);
			...
			e._getPersonalizationDetails("keyATPOverviewEnquiry")
		},
		...
		valueHelpRequestPlant: function (e) {
			...
			if (t.plantDataAccess === "No Access") {
				...
				var t = this;
				if (!t.plantFrag) {
					t.plantFrag = sap.ui.xmlfragment("dksh.connectclient.InventoryAtp.Fragments.plant", t);
		...
		valueHelpRequestMaterialGrp4: function (e) {
			...
			if (t.materialGroup4DataAccess === "No Access") {
				...
				if (!t.materialGroup4) {
					t.materialGroup4 = sap.ui.xmlfragment("dksh.connectclient.InventoryAtp.Fragments.materialGroup4", t);
		...
		valueHelpRequestMaterialGrp: function (e) {
			...
			if (t.materialGroupDataAccess === "No Access") {
				...
				if (!t.MaterialGroup) {
					t.MaterialGroup = sap.ui.xmlfragment("dksh.connectclient.InventoryAtp.Fragments.MaterialGroup", t);
		...
		valueHelpRequestStoLoc: function (e) {
			...
			if (l.SLOCDataAccess === "No Access") {
				...
				l.StorageLocation = sap.ui.xmlfragment("dksh.connectclient.InventoryAtp.Fragments.StorageLocation", l);
		...
		onVariantDelete: function () {
			...
			var o = {
				...
				applicationId: "keyATPOverviewEnquiry",
		...
		onExportATPoverview: function (JSONData, ReportTitle, ShowLabel) {
			...
			var defaultVariant = that.getView().getModel("PersonalizationModel").getProperty("/personalizationData/userPersonaDto");
			// var cols = [];
				...
				for (var k = 0; k < defaultVariant.length; k++) {
					...
					if (defaultVariant[k].status === true) {
						if (defaultVariant[k].enabledKey === "Material") {
							...
							row = row + o.convertToDateToDispFormat(arrData[i].expiredDate) + '","';
						} else if (defaultVariant[k].enabledKey === "Req. Qty") {
							...
							var shortQtyReport = o.shortQty(arrData[i].shortQty);
		...
		ATPOverviewSearch: function () {
			...
			// that._getPersonalizationDetails("keyATPOverviewEnquiry"); jj
			// sales Org
			if (this.salesOrgFromSelectedItems.length === 0) {
		...
		popOver: function (e) {
			...
			if (!this._oPopover) {
				r.load({
					name: "dksh.connectclient.InventoryAtp.Fragments.popoverTable",
					controller: this
				}).then(function (e) {
					this._oPopover = e;
					this.getView().addDependent(this._oPopover);
		...
		navigateStkLot: function (e) {
			if (sap.ui.Device.system.phone === true) {
				...
				if (!this._oPopover) {
					r.load({
						name: "dksh.connectclient.InventoryAtp.Fragments.StockLotPopover",
						controller: this
					}).then(function (e) {
						this._oPopover = e;
						this.getView().addDependent(this._oPopover);
		...
		valueHelpRequestSalesOrg: function (e) {
			...
			if (t.salesOrgDataAccess === "No Access") {
				...
				if (!t.salesOrg) {
					t.salesOrg = sap.ui.xmlfragment("dksh.connectclient.InventoryAtp.Fragments.SalesOrg", t);
